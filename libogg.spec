...
Source0  : https://downloads.xiph.org/releases/ogg/libogg-1.3.6.tar.xz
...
%prep
%setup -q -n libogg-1.3.6
cd %{_builddir}/libogg-1.3.6
%patch -P 1 -p1
pushd ..
cp -a libogg-1.3.6 build32
popd
pushd ..
cp -a libogg-1.3.6 buildavx2
popd
pushd ..
cp -a libogg-1.3.6 buildavx512
popd
pushd ..
cp -a libogg-1.3.6 buildapx
popd
...
export LANG=C.UTF-8
export SOURCE_DATE_EPOCH=1750100730
...
CXXFLAGS="$CLEAR_INTERMEDIATE_CXXFLAGS -march=x86-64-v4 -mapxf -Wl,-z,x86-64-v4 "
...
%files lib
%defattr(-,root,root,-)
/V3/usr/lib64/libogg.so.0.8.6
/V4/usr/lib64/libogg.so.0.8.6
/VA/usr/lib64/libogg.so.0.8.6
/usr/lib64/libogg.so.0
/usr/lib64/libogg.so.0.8.6

%files lib32
%defattr(-,root,root,-)
/usr/lib32/libogg.so.0
/usr/lib32/libogg.so.0.8.6
